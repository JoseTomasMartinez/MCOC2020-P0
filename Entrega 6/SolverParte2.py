...
solvers=["Inversa","NumpySolver","ScipySolver","SpSolverSymmetric","SpSolverPos","SpSolverPosOverwrite"]
Corridas=10
N = [2,5,10,12,15,20,30,40,45,50,55,60,75,100,125,160,200,250,350,500,600,800,1000,2000,3000,5000,7000,10000]
...
for i in range(len(N)):
	...
	for j in range(Corridas):
		...
		#Scipy Solver Symmetric
		A=matriz_laplaciana(N[i])
		b=ones(N[i])
		t1=perf_counter()
		x=sp.linalg.solve(A,b,sym_pos=True)
		t2=perf_counter()
		dt=t2-t1
		dts[j][3]=dt

		#Scipy Solver Positive
		A=matriz_laplaciana(N[i])
		b=ones(N[i])
		t1=perf_counter()
		...
		#Scipy Solver Positive Overwrite
		A=matriz_laplaciana(N[i])
		b=ones(N[i])
		t1=perf_counter()
		x=sp.linalg.solve(A,b,assume_a="pos",overwrite_a=True,overwrite_b=True)
		t2=perf_counter()
		dt=t2-t1
		dts[j][5]=dt
...
for i in range(len(solvers)):
	plt.plot(N,T[i],"o-")
plt.legend(solvers)
plt.savefig("Rendimiento Solvers Parte2")	
plt.show()